//! Transactional outbox for the Basket bounded context.
//!
//! Mirror of `catalog::integration_event_log` and
//! `ordering-infrastructure::integration_event_log`, scoped to
//! basket's own [`BasketIntegrationEventLogRow`] so the three
//! services own independent tables.

use chrono::{DateTime, Utc};
use uuid::Uuid;

use crate::error::Error;
use crate::row::BasketIntegrationEventLogRow;
...
impl EventState {
    ...
    ///
    /// # Errors
    /// Returns [`Error::InvalidPersistedValue`] if `s` is not one of
    /// the known states.
    pub fn parse_persisted(s: &str) -> Result<Self, Error> {
        match s {
            ...
            other => Err(Error::InvalidPersistedValue {
                context: "basket_integration_event_logs.state".to_string(),
...
/// Caller-supplied row to insert into the outbox.
#[derive(Debug, Clone)]
...
fn row_to_log(row: BasketIntegrationEventLogRow) -> Result<EventLog, Error> {
    Ok(EventLog {
...
impl IntegrationEventLogService {
    ...
    where
        E: toasty::Executor,
    {
        toasty::create!(BasketIntegrationEventLogRow {
    ...
    /// Insert a batch of outbox rows in a single transaction.
    /// Localized FFI exception (see catalog / ordering-infrastructure
    /// counterparts).
    ///
    /// # Errors
    ...
    /// Retrieve every [`EventState::NotPublished`] row across the
    /// entire outbox, regardless of `transaction_id`.  Used by the
    /// publisher worker.
    ///
    /// # Errors
    /// Propagates toasty and time-conversion errors.
        ...
        E: toasty::Executor,
    {
        ...
        let rows: Vec<BasketIntegrationEventLogRow> = toasty::query!(
            BasketIntegrationEventLogRow FILTER .state == #target_state
        )
        .exec(executor)
        .await?;
    ...
    where
        E: toasty::Executor,
    {
        let row = BasketIntegrationEventLogRow::get_by_event_id(executor, &event_id).await?;
        let new_times_sent = if bump_times_sent {
            row.times_sent.saturating_add(1)
        } else {
            row.times_sent
        };
        BasketIntegrationEventLogRow::update_by_event_id(event_id)
