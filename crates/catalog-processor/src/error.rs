//! Crate error type for the catalog outbox publisher worker.

/// Errors produced by the catalog outbox publisher worker.
#[derive(Debug)]
pub enum Error {
    /// Raw toasty driver failure.
    Toasty { reason: String },
    /// Any `catalog` crate error (persistence, mapping, validation).
